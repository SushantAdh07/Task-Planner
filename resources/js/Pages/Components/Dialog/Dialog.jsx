...
function Dialog({ value, tasks, loggedInUser, selectedUser, users, errors, comments }) {
    ...
    return (
        <div className="flex flex-col md:flex-row h-full">
            {/* Sidebar - Task List */}
            <div className={`${mobileView && (inputDialog || editDialog) ? 'hidden' : 'block'} w-full md:w-1/3 bg-blue-800 text-white p-4 rounded-t-lg md:rounded-l-lg md:rounded-tr-none`}>
                            ...
                            <li key={task.id}>
                                ...
                                >
                                    <div className="flex justify-between items-center">
                                        ...
                                        {task.status === 1 && (
                                            <span className="text-xs bg-green-500 text-white px-2 py-1 rounded-full">Done</span>
                                        )}
                                    </div>
                                </button>
                            </li>
            ...
            {/* Main Content Area */}
            <div className={`${mobileView && (inputDialog || editDialog) ? 'block' : 'display'} md:block flex-1 bg-white p-6 rounded-b-lg md:rounded-r-lg md:rounded-bl-none overflow-y-auto`}>
                {inputDialog ? (
                    ...
                    <div className="space-y-6">
                        <div>
                            <p className="text-sm text-gray-500">
                                {users.find(user => user.id === selectedUser)?.name} • {value}
                            </p>
                        </div>
                        <h3 className="text-xl font-bold text-gray-800">{selectedTask.task_name}</h3>
                        ...
                        {selectedTask.status === 1 && (
                            <div className="flex items-center text-green-600">
                                ...
                                <span>This task is marked as completed</span>
                            </div>
                        )}
                    ...
                    <div className="text-center py-8">
                        ...
                        <h3 className="text-lg font-medium text-gray-800 mb-2">No task selected</h3>
                        <p className="text-gray-500 mb-4">Select a task from the list or create a new one</p>
                        <button
                            onClick={openInputDialog}
                    ...
                    <div className="text-center py-8">
                        ...
                        <p className="text-gray-500">This user has no tasks for this date</p>
                    </div>
                )}
            </div>
        </div>
