...
export default function TeamSidebar() {
  ...
  return (
    <div className={`h-screen bg-gradient-to-b from-slate-950/95 via-blue-950/95 to-slate-950/95 backdrop-blur-xl border-r border-white/10 transition-all duration-300 font-['Poppins',sans-serif] ${isCollapsed ? 'w-16' : 'w-64'} relative`}>
            ...
            {menuItems.map((item) => (
              ...
              >
                <div className={`flex-shrink-0 transition-all duration-300 ${
                  activeItem === item.id ? 'text-blue-400' : 'group-hover:text-white group-hover:scale-110'
                }`}>
                  {item.icon}
                </div>
          ...
          <div className="space-y-2">
            {quickActions.map((action) => (
              ...
              >
                ...
                {!isCollapsed && (
                  <span className="ml-3 tracking-wide">{action.name}</span>
                )}
              </button>
            ))}
          </div>
      ...
      <div className="p-4 border-t border-white/5">
        <div className={`${isCollapsed ? 'flex justify-center' : 'flex items-center justify-between'}`}>
          {!isCollapsed && (
          ...
          <button className={`${isCollapsed ? 'w-8 h-8' : 'px-3 py-2'} bg-gradient-to-r from-blue-600 to-cyan-600 rounded-lg text-white text-xs font-medium hover:shadow-lg hover:shadow-blue-500/25 transition-all duration-300 hover:scale-105`}>
            {isCollapsed ? '↗' : 'Upgrade'}
          </button>
        </div>
      </div>
